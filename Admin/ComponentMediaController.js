...
module.exports = (supabase, supabaseStorage) => {
  ...
  router.post('/components', authenticateToken, requireAdmin, upload.single('image'), async (req, res) => {
    console.log('Received request to add component');
    const { componentmedia_name, thaiinstrument_id } = req.body;
    const file = req.file;

    console.log('Data:', { componentmedia_name, thaiinstrument_id, fileName: file?.originalname });
    ...
    console.log('Inserting into database:', newComponent);
    ...
    console.log('✅ Component inserted successfully:', data[0]);
